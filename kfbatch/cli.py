...
def _build_parser():
    ...
    parser.add_argument('--slurm_reservation_command', metavar='command', default='scontrol show reservation', type=str, required=False, action='store',
                        help='default=%(default)s: Command for active SLURM reservations.')
    parser.add_argument('--slurm_reservation_example_file', metavar='PATH', default='', type=str, required=False, action='store',
                        help='default=%(default)s: PATH to a file with --slurm_reservation_command stdout.')
    parser.add_argument('--slurm_prio_command', metavar='command', default='sprio', type=str, required=False, action='store',
    ...
    parser.add_argument('--exclude_abnormal_node', metavar='[yes,no]', default='yes', type=parse_bool, required=False, action='store',
...
def main(argv=None):
    ...
    from kfbatch.stat import KFBatchError, stat_main
    try:
        stat_main(args)
