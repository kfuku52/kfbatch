...
- Share compact table column sizing and row printing between Slurm and Grid Engine
  summaries, preserving existing output formatting and scheduler calculations.

## [0.4.9] - 2026-09-22

### Fixed

- Preserve inode counts when an aligned quota table has one numeric grace value
  and one empty grace field, including wrapped filesystem rows. Reject ambiguous
  unaligned numeric rows with a diagnostic instead of silently shifting columns.

## [0.4.8] - 2026-09-22

### Changed

- Align installation prerequisites, CLI help, job-count semantics, quota input,
  and TSV interpretation/write behavior with the current implementation.
- Add an offline usage reference with complete Grid Engine and quota examples;
  include it in source distributions. Scheduler behavior is unchanged.
...
- Update CodeQL initialization and analysis together to 4.37.9, and group
  future CodeQL Dependabot updates to prevent incompatible action versions.

## [0.4.2] - 2026-07-31

### Fixed

- Parse SHIROKANE `lfsq` `Gbytes` and `kfiles` columns at their declared scales,
  including comma-separated limits, and distinguish an unrecognized successful
  response from a command failure that may require `qlogin`.
...
[0.4.2]: https://github.com/kfuku52/kfbatch/compare/v0.4.1...v0.4.2
